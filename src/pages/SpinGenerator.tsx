...
type Phase = "idle" | "spinning" | "slowing" | "winner";

function LightDot({ index, isWon }: { index: number; isWon: boolean }) {
  return (
    <motion.div
...
export default function SpinGenerator() {
  const [phase, setPhase] = useState<Phase>("idle");
  const [leverPulled, setLeverPulled] = useState(false);

  // Pick a random winner from fake names
  const winnerName = useMemo(() => {
    const randomIndex = Math.floor(Math.random() * FAKE_NAMES.length);
    return FAKE_NAMES[randomIndex];
  }, []);
            ...
            >
              ...
              <div className="flex justify-center gap-3 py-2.5 w-full px-4">
                {Array.from({ length: lightCount }).map((_, i) => (
                  <LightDot key={`top-${i}`} index={i} isWon={isWon} />
                ))}
              </div>
              ...
              <div className="flex justify-center gap-3 py-2.5 w-full px-4">
                {Array.from({ length: lightCount }).map((_, i) => (
                  <LightDot key={`bot-${i}`} index={i + lightCount} isWon={isWon} />
                ))}
              </div>
          ...
          {phase === "winner" && (
            ...
            >
              <p className="text-lg font-bold text-foreground">
                wins <span className="text-primary">$x</span>
              </p>
